# Koop housing price data (chapter 4)
# 
...
par(mfrow=c(2,3))
i <- 2
  ts.plot(bs[,i],ylab="",xlab="iteration",main=paste("beta",i,sep=""),col=4)
  acf(bs[,i],ylab="",main="",col=2,lwd=3)
  hist(bs[,i],prob=T,xlab="",ylab="",main="",col=3)

  ts.plot(s2,ylab="",xlab="iteration",main=paste("sig2",sep=""),col=4)
  acf(s2,ylab="",main="",col=2,lwd=3)
  hist(s2,prob=T,xlab="",ylab="",main="",col=3)


par(mfrow=c(2,2))
for (i in 2:4) { plot(density(b[,i]),col="blue", main = cbind("Beta",i))
 }
...
# see p.84 of Koop for similar results - try his prior
