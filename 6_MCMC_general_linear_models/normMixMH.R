function (theta0, theta1, p, candidate, steps = 1000, type = "ind", 
    randomSeed = NULL, startValue = NULL) 
{
    ...
    if (type == "rw") {
        ...
        k2 <- (1 - p)/sigma1 * exp(-0.5 * ((targetSample[1] - 
            mu1)/sigma1)^2)
        g[1] <- k1 + k2
        i1 <- 1
        for (n in 2:steps) {
            ...
            k2 <- (1 - p)/sigma1 * exp(-0.5 * ((proposal[n] - 
                mu1)/sigma1)^2)
            g[n] <- k1 + k2
            k3 <- g[n]
            k4 <- g[i1]
        ...
        i1 <- 1
        for (n in 2:steps) {
            alpha[n] <- density0[i1] * densityMix[n]/(density0[n] * 
                densityMix[i1])
    ...
    hist(targetSample, prob = TRUE, col = "light blue", xlim = range(theta), 
