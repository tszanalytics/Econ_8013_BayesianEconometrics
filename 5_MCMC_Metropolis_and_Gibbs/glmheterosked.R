## linear reg with heteroskedastic errors (t-distributed errors)
rm(list=ls())
...
# pdf(file="hetero-error1.pdf",width=10,height=8)
...
for (iter in 1:niter){
  ...
  #### INCORRECT (as an example of nonconvergence) - REMOVE for correct answers
#  if (iter == 1) {
#    b  = b1+t(chol(V1))%*%rnorm(p)
#  }
#  else {
#  b  = 0.03*(b1+t(chol(V1))%*%rnorm(p)) + 0.97*bs[(iter-1),]
#  }
  ####
...
for (i in 1:p){
  ...
  acf(bs[,i],ylab="",main="")
  hist(bs[,i],prob=T,xlab="",ylab="",main="",breaks=50)
...
for (i in 1:p){
  ...
  acf(as[,i],ylab="",main="")
  hist(as[,i],prob=T,xlab="",ylab="",main="",breaks=50)
  abline(v=alpha[i],col=2)
}
# dev.off()

plot(density(bs[,1]),col=4)
plot(density(bs[,2]),col=4)
plot(density(as[,1]),col=4)
plot(density(as[,2]),col=4)
